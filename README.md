...
| **Scoped** | 17 purpose-built profiles, narrowable to sub-profiles, plus a `--domains` flag. Load only what a project needs, so large tool sets don't overwhelm your client. |
...
One install backs seventeen small, purpose-built MCP servers — `app-info`, `distribution`, `monetization`, `game-center` and thirteen more. Register only the areas a project uses: the full 982-operation surface would cost well over 100k tokens of tool definitions, while one profile costs a fraction of that. A big profile narrows further — `monetization:subscriptions` is 108 tools instead of 201.
...
Tek kurulum, on yedi küçük, amaca özel MCP sunucusu sunar — `app-info`, `distribution`, `monetization`, `game-center` ve on üç tane daha. Sadece projenizin kullandığı alanları kaydedin: tüm 982 işlemlik yüzey, araç tanımları için 100 bin token'ın epey üzerinde bir maliyet çıkarır; tek bir profil ise bunun küçük bir kısmı. Büyük bir profil daha da daralır — `monetization:subscriptions` 201 yerine 108 araç.
