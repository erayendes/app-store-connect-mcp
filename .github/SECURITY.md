...
- **Confirm before risky writes (on by default)** — before a `revenue`, `destructive`, `infrastructure` or `access` write runs, the server asks the user to confirm via [MCP elicitation](https://modelcontextprotocol.io/), showing what would change, so a misread instruction can't execute unchecked. Writes below those levels rely on the client's own per-call tool approval, which is the gate that always exists. The split is deliberate: asking on every write meant that a client which declares elicitation support but can't render the form answered `decline` — identical, in the protocol, to a user refusing — so ordinary writes failed as "you refused"; asking on none removed the guard from the writes that move money. `--confirm` / `ASC_CONFIRM_WRITES=1` asks before every write, `--no-confirm` / `ASC_CONFIRM_WRITES=0` before none. With the guard on, a client that never declared elicitation **fails closed**. For a hard stop that depends on no client behaviour at all, use `--read-only`.
...
- **Riskli yazmalardan önce onay (varsayılan açık)** — bir `revenue`, `destructive`, `infrastructure` ya da `access` yazması çalışmadan önce sunucu kullanıcıdan [MCP elicitation](https://modelcontextprotocol.io/) ile onay ister ve neyin değişeceğini gösterir; böylece yanlış anlaşılmış bir talimat kontrolsüz çalışamaz. Bu seviyelerin altındaki yazmalar client'ın kendi çağrı-başı araç onayına dayanır — her zaman var olan kapı odur. Ayrım bilinçli: her yazmada sormak, elicitation desteğini bildirip formu gösteremeyen bir client'ın `decline` dönmesi demekti — protokol düzeyinde bu, kullanıcının reddetmesiyle birebir aynı — ve sıradan yazmalar "reddettiniz" diye düşüyordu; hiç sormamak ise guard'ı parayı hareket ettiren yazmalardan kaldırdı. `--confirm` / `ASC_CONFIRM_WRITES=1` her yazmadan önce sorar, `--no-confirm` / `ASC_CONFIRM_WRITES=0` hiç sormaz. Guard açıkken, elicitation'ı hiç bildirmemiş bir client'ta **fail-closed** davranır. Hiçbir client davranışına bağlı olmayan kesin durdurma için `--read-only` kullanın.
