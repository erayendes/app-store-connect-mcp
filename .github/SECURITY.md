...
- **Confirm before writes (opt-in, `--confirm` / `ASC_CONFIRM_WRITES=1`)** — before any mutating tool runs, the server asks the user to confirm via [MCP elicitation](https://modelcontextprotocol.io/), showing what would change, so a misread instruction can't execute unchecked. It is **off by default**, because the gate that always exists is your client's own per-call tool approval, and this one only works where the client renders an elicitation form: a client that declares the capability but can't show the form answers `decline` — identical, in the protocol, to a user refusing. Turn it on once you have seen the prompt appear in your client. With it on, a client that never declared elicitation **fails closed**: writes are blocked rather than proceeding under a guard that isn't running. For a hard stop that depends on no client behaviour at all, use `--read-only`.
...
- **Yazmadan önce onay (opt-in, `--confirm` / `ASC_CONFIRM_WRITES=1`)** — değişiklik yapan bir araç çalışmadan önce, sunucu kullanıcıdan [MCP elicitation](https://modelcontextprotocol.io/) ile onay ister ve neyin değişeceğini gösterir; böylece yanlış anlaşılmış bir talimat kontrolsüz çalışamaz. **Varsayılan kapalı**, çünkü her zaman var olan kapı client'ınızın kendi çağrı-başı araç onayı; bu guard ise yalnızca client elicitation formunu gerçekten gösterebiliyorsa çalışır. Yeteneği bildirip formu gösteremeyen bir client `decline` döner — protokol düzeyinde bu, kullanıcının reddetmesiyle birebir aynıdır. İstemin client'ınızda göründüğünü gördükten sonra açın. Açıkken, elicitation'ı hiç bildirmemiş bir client'ta **fail-closed** davranır: çalışmayan bir guard'ın altından geçmek yerine yazma engellenir. Hiçbir client davranışına bağlı olmayan kesin durdurma için `--read-only` kullanın.
