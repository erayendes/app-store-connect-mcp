...
- **Confirm before writes (on by default)** — before any mutating tool runs, the server asks the user to confirm via [MCP elicitation](https://modelcontextprotocol.io/), so a misread instruction can't execute unchecked. Disable with `ASC_CONFIRM_WRITES=0` / `--no-confirm`. On clients without elicitation support the guard **fails closed**: writes are blocked with an error rather than silently proceeding. To allow writes on such a client anyway, opt in explicitly with `--allow-unconfirmed-writes` / `ASC_ALLOW_UNCONFIRMED_WRITES=1` — they then rely on the client's own per-call approval, so pair that with `--read-only` or a narrow scope where you can.
...
- **Yazmadan önce onay (varsayılan açık)** — değişiklik yapan bir araç çalışmadan önce, sunucu kullanıcıdan [MCP elicitation](https://modelcontextprotocol.io/) ile onay ister; böylece yanlış anlaşılmış bir talimat kontrolsüz çalışamaz. `ASC_CONFIRM_WRITES=0` / `--no-confirm` ile kapatılır. Elicitation desteklemeyen client'larda guard **fail-closed** çalışır: yazmalar sessizce geçmek yerine hata ile engellenir. Böyle bir client'ta yine de yazmaya izin vermek için açıkça `--allow-unconfirmed-writes` / `ASC_ALLOW_UNCONFIRMED_WRITES=1` ile opt-in yapın — yazmalar o zaman client'ın kendi çağrı-başı onayına dayanır; mümkünse `--read-only` veya dar kapsam ile birlikte kullanın.
