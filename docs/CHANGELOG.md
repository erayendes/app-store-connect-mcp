...
- **`asc__load` adds a sub-profile mid-session**, without a restart. Measured before shipping: Claude Code hands the revised tool list to the model inside the same turn and it just works; Codex only refreshes between turns, and the model there is left calling tools it cannot see. So the reply carries the loaded tools and their descriptions itself, and tells the model to ask the user to repeat the request rather than call something invisible. Hand-written families (StoreKit, reviews-AI, pricing macros) still need a restart, and the reply says so.
...
- **`asc__load` bir alt profili oturum ortasında ekliyor**, yeniden başlatmadan. Yayınlamadan önce ölçüldü: Claude Code yenilenen araç listesini modele aynı turda veriyor, sorunsuz çalışıyor; Codex listeyi yalnızca turlar arasında yeniliyor ve model orada göremediği araçları çağırmaya kalkıyor. Bu yüzden cevap, yüklenen araçları ve açıklamalarını kendisi taşıyor ve modele, görünmeyen bir aracı çağırmak yerine kullanıcıdan isteği tekrarlamasını istemesini söylüyor. Elle yazılmış aileler (StoreKit, reviews-AI, fiyat makroları) hâlâ yeniden başlatma istiyor; cevap bunu da söylüyor.
