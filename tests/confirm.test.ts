...
describe('confirmWrite', () => {
  it('fails closed when the client has no elicitation support', async () => {
    const { c, elicit } = confirmer({});

    const decision = await confirmWrite(c, 'subscriptions__update');
  ...
  it('blocks and reports the action when the user declines', async () => {
    ...
    const decision = await confirmWrite(c, 'app_store_versions__delete');
