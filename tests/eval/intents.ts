...
export const INTENTS: EvalIntent[] = [
  ...
  // marketing (4)
  // Read `foundTarget` on this one with a caveat: `app_screenshots.create`
  // reserves a slot and moves no bytes, so it is step 1 of 4 and reaching it
  // does not mean the screenshot went up. `expectedTool` has to name a spec
  // operation (the corpus invariant below enforces that), so the honest signal
  // here is `usedMacro` — listing__upload_screenshot is the whole job.
  { intent: 'Upload a screenshot to the store listing', searchQuery: 'upload app store screenshot listing', expectedTool: 'app_screenshots.create', macro: 'listing__upload_screenshot', chain: [...versionChain, { op: 'app_store_versions.app_store_version_localizations.list', path: '/v1/appStoreVersions/{id}/appStoreVersionLocalizations', idFrom: '0:data[0].id' }, { op: 'app_store_version_localizations.app_screenshot_sets.list', path: '/v1/appStoreVersionLocalizations/{id}/appScreenshotSets', idFrom: '1:data[0].id' }], phrasings: ['Mağaza sayfasına ekran görüntüsü yükle', 'App Store screenshot ekle', 'Upload a store listing screenshot', 'Add a screenshot to the App Store page'] },
  ...
  // game-center (3)
  { intent: 'Create a Game Center achievement', searchQuery: 'add a new player milestone', expectedTool: 'game_center_achievements_v2.create', phrasings: ['Game Center başarımı oluştur', 'Oyuna yeni achievement ekle', 'Create a Game Center achievement', 'Add a new game achievement'] },
  { intent: 'Create a Game Center leaderboard', searchQuery: 'add a ranked score board', expectedTool: 'game_center_leaderboards_v2.create', phrasings: ['Game Center liderlik tablosu oluştur', 'Yeni leaderboard ekle', 'Create a Game Center leaderboard', 'Add a score leaderboard'] },
